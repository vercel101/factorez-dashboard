...
const CancelledOrder = ({ tokenReducer, userInfoReducer }) => {
    ...
    const getAllCancelledOrder = async () => {
        await getAllCancelledOrderAPI(tokenReducer).then(res => {
                ...
                <tbody>
                    {orders.length > 0 ? orders.map((el, i) => (
                        <tr key={el._id} className={`bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600`}>
                            ...
                            <td className={`px-6 py-1`}>{(Object.keys(el.order_status_id.cancelled.userId)[0])[0].toUpperCase()+Object.keys(el.order_status_id.cancelled.userId)[0].substring(1)}</td>
                            <td className={`px-6 py-1`}>
                                <div
                                    className={`border 
                                                flex p-1 rounded 
