...
const OrderStatusModel = ({ data, close, updatetrackingno, userInfoReducer, updateOrder, changeOrderStatus, questions }) => {
    ...
    const changeOrderStatusFn = () => {
        if (newOrderStatus) {
            changeOrderStatus(data.orderInfo._id, newOrderStatus);
        }
    };
    ...
    useEffect(() => {
        for (let x of data.orderInfo.order_status_id.statusList) {
            setStatusList((old) => [...old, x.status]);
        }
    }, []);
