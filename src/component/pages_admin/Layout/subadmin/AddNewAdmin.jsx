        ...
        <div>
            ...
            <div className="grid grid-cols-4 gap-2">
                ...
                <div>
                    ...
                    <select
                        className="w-full p-2 outline-none bg-[#fafafa] border rounded-md dark:bg-[#424242] dark:border-[#424242]"
                        value={newAdminTextField.gender}
                        onChange={(e) =>
                            setNewAdminTextField((prevState) => {
            ...
            <div className={`border p-4 bg-green-100 dark:bg-[#424242] dark:border-[#424242] rounded-md`}>
                <div className="grid grid-cols-4 gap-2">
                                    ...
                                    {typeof allEnums[el] === "string" && (
                                        <>
                                            <li key={`${i}_otherFields`}>
                                                <input type="checkbox" defaultChecked={adminRoles.includes(allEnums[el])} onChange={(e) => addRolesToStack(allEnums[el])} />{" "}
                    ...
                    {Object.keys(allEnums).map((el, i) => (
                        <>
                            {typeof allEnums[el] !== "string" && (
                                <div key={`${i}_${el}`} className="bg-white dark:bg-[#242424]">
                                    <h1 className="font-bold text-lg dark:bg-teal-800 bg-teal-300 p-2">{el}</h1>
                                    <ul className="ps-2">
                                        {Object.keys(allEnums[el]).map((subEl, subi) => (
                                            <li key={`${subi}_${subEl}`}>
                                                <input
                                                    type="checkbox"
            ...
            <div className={`mt-2`}>
                <button className={`border px-2 py-1 bg-[#4f46e5] rounded-md text-white hover:bg-indigo-800`} onClick={() => createNewAdminFn()}>
                    Submit
                </button>
                <button className={`ms-1 border px-2 py-1 rounded-md bg-red-300 hover:bg-yellow-200`} onClick={() => cancelNewAdminFn()}>
                    Cancel
                </button>
            </div>
        </div>
